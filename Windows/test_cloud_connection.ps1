# Windows 平台雲端端點連接測試腳本 (PowerShell)
...
function Test-BasicConnectivity {
    Write-Host "`n🔗 測試基本連接性..." -ForegroundColor Cyan
...
function Test-HealthEndpoint {
    Write-Host "`n🏥 測試健康檢查端點..." -ForegroundColor Cyan
...
function Test-DoctorAuthentication {
    Write-Host "`n👨‍⚕️ 測試醫師認證端點..." -ForegroundColor Cyan
...
function Test-AnnotationUpload {
    Write-Host "`n📝 測試標註資料上傳端點..." -ForegroundColor Cyan
...
function Test-ImageUpload {
    Write-Host "`n🖼️ 測試影像上傳端點..." -ForegroundColor Cyan
    
    try {
        # 創建測試影像檔案
        $TestImagePath = "test_upload_image.jpg"
        if (-not (Test-Path $TestImagePath)) {
            Create-TestImage -Path $TestImagePath
        }
        ...
        $BodyLines = @(
            ...
            "test_patient_001",
            "--$Boundary",
            "Content-Disposition: form-data; name=`"annotation_id`"",
            "",
            "test_annotation_001",
            "--$Boundary",
            "Content-Disposition: form-data; name=`"image`"; filename=`"test_wound.jpg`"",
        ...
        $Stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
        $Response = Invoke-WebRequest -Uri "$BaseUrl/upload/image" -Method POST -Body $Body -Headers $Headers -TimeoutSec 30 -UseBasicParsing
        $Stopwatch.Stop()
        
        if ($Response.StatusCode -in @(200, 201, 422)) {
            Write-TestResult -TestName "影像上傳端點" -Success $true -Message "端點可達 (狀態碼: $($Response.StatusCode))" -ResponseTime $Stopwatch.Elapsed.TotalSeconds
            return $true
        } else {
            Write-TestResult -TestName "影像上傳端點" -Success $false -Message "端點錯誤 (狀態碼: $($Response.StatusCode))" -ResponseTime $Stopwatch.Elapsed.TotalSeconds
            return $false
        }
    }
    catch {
        Write-TestResult -TestName "影像上傳端點" -Success $false -Message "請求錯誤: $($_.Exception.Message)"
        return $false
    }
}
...
function Create-TestImage {
    ...
    try {
        # 創建一個簡單的測試檔案
        "Test image content for upload testing" | Out-File -FilePath $Path -Encoding UTF8
        Write-Host "✓ 創建測試檔案: $Path" -ForegroundColor Green
    }
    catch {
        Write-Host "✗ 創建測試檔案失敗: $($_.Exception.Message)" -ForegroundColor Red
    }
}

function Test-NetworkPerformance {
    Write-Host "`n⚡ 測試網路效能..." -ForegroundColor Cyan
...
function Main {
    ...
    # 執行所有測試
    $Tests = @(
        @{ Name = "基本連接測試"; Function = "Test-BasicConnectivity" }
        @{ Name = "健康檢查端點"; Function = "Test-HealthEndpoint" }
        @{ Name = "醫師認證端點"; Function = "Test-DoctorAuthentication" }
        @{ Name = "標註上傳端點"; Function = "Test-AnnotationUpload" }
        @{ Name = "影像上傳端點"; Function = "Test-ImageUpload" }
        @{ Name = "網路效能測試"; Function = "Test-NetworkPerformance" }
    )
    ...
    foreach ($Test in $Tests) {
        try {
            ...
            Write-TestResult -TestName $Test.Name -Success $false -Message "測試執行錯誤: $($_.Exception.Message)"
        }
    }
