...
# 註:純色填滿整圖會使灰世界白平衡退化(production 為完整照片);純色 v1 規則單元測試用 use_v2=False。v2 由 test_tissue_severity + 真實照片驗證。
r=[]
...
ck("紅 -> 肉芽為主", classify(red,mask,use_v2=False)["tissue_dominant"]=="肉芽為主")
ck("黑 -> 壞死為主", classify(blk,mask)["tissue_dominant"]=="壞死為主")
ck("黃 -> 腐肉為主", classify(yel,mask,use_v2=False)["tissue_dominant"]=="腐肉為主")
pink=np.zeros((H,W,3),np.uint8); pink[...,0]=228; pink[...,1]=172; pink[...,2]=168  # 淡粉上皮
ck("淡粉 -> 上皮為主", classify(pink,mask)["tissue_dominant"]=="上皮為主")
ck("純紅非上皮(仍肉芽)", classify(red,mask,use_v2=False)["tissue_dominant"]=="肉芽為主")
