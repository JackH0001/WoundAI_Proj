...
class DeploymentScriptSafetyTests(unittest.TestCase):
    def test_deploy_is_clean_explicit_and_non_mutating_iam(self):
        ...
        self.assertIn("$AuditBucket -eq 'woundai-flywheel-jackh001-audit-epoch-20260905'", src)
        self.assertIn("AuditBucket was retired after the 2026-09-06 test contamination", src)
        self.assertIn("$candidate -cnotmatch '^[a-z0-9]", src)
