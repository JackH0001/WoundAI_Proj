...
| **稽核軌跡雜湊鏈** | 每筆稽核紀錄攜帶前一筆的 SHA-256；竄改／刪除／調換順序皆可偵測並指出斷點 | `python engineering/phase2/verify_audit_chain.py` |
...
建一個**獨立的** `-audit` 桶並設 7 年保留政策。為什麼要分桶：GCS 的保留政策是桶層級、
...
雲端桶目前是空的（`臨床 0/20`），這就是正確的起點。如果在測試期間送過標註進去，
正式收案前要清掉，否則收案進度的分母是假的：

```powershell
gcloud.cmd storage rm -r "gs://woundai-flywheel-jackh001/flywheel/retrain_queue.jsonl/"
gcloud.cmd storage rm -r "gs://woundai-flywheel-jackh001/flywheel/images/"
```

**不要刪 `audit.jsonl/`** —— 稽核軌跡要連續，斷在中間比雜訊更糟。
